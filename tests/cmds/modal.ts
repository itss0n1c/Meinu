...
export default new Command({
	...
	new Command({
		...
		.addHandler('button', async (bot, int) => {
			const modal = new ModalBuilder().setCustomId('modal-one').setTitle('Modal');
			...
			await int.channel?.send({
				components: [
					new ActionRowBuilder<ButtonBuilder>().addComponents(
						new ButtonBuilder().setCustomId('modal-one-modal').setLabel('Open modal').setStyle(ButtonStyle.Primary)
					)
				]
			});
